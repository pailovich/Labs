...
{
  for(int i = 0; i < cvector_size(matrix); i++)
  {
    for(int j = cvector_size(matrix) - 1; j > i; j--)
    {
      if((*(int*)cvector_get(ind, j-1)) > (*(int*)cvector_get(ind, j)))
      {
	float temp = *(float*)cvector_get(matrix, j-1);
	cvector_set(matrix, j-1, (float*)cvector_get(matrix, j));
	cvector_set(matrix, j, &temp);
	int helper = *(int*)cvector_get(ind, j-1);
	cvector_set(ind, j-1, (int*)cvector_get(ind, j));
	cvector_set(ind, j, &helper);
      }
    }
  }
}
